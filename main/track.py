...
class Track:
    ...
    def idrequest(self, identity: int, location: Optional[Tuple[float, float]] = None, _retry_count: int = 0) -> requests.Response:
        """Request member info by ID. Retries up to 2 times on 401 errors."""
        MAX_RETRIES = 2
        ...
        try:
            ...
            if r.status_code == 401:
                if _retry_count >= MAX_RETRIES:
                    print(f'Max retries ({MAX_RETRIES}) reached for idrequest. Returning 401.')
                    return r
                print(f'Unauthorized, relogging (attempt {_retry_count + 1}/{MAX_RETRIES})')
                self.token = self.login()
                return self.idrequest(identity, location, _retry_count + 1)

            return r
        except requests.RequestException as e:
            print(f"ID request failed: {e}")
            # Return a dummy response to avoid crash
    ...
    def optimizeAccuracy(self, name: str, location80: Tuple[float, float]) -> Tuple[float, float]:
        ...
        for dx in drange:
            for dy in drange:
                # Fixed: was incorrectly concatenating list + array which produced [dx, 0, 0, dy]
                probe = np.array(location80) + np.array([dx, dy])
                probes.append(probe)
